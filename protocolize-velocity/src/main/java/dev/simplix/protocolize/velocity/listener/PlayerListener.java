...
public class PlayerListener {
    ...
    private static Field serverConnectionField = ReflectionUtil.fieldOrNull(ConnectedPlayer.class, "connectionInFlight", true);
    private static Field minecraftConnectionField = ReflectionUtil.fieldOrNull(InitialInboundConnection.class, "connection", true);
    ...
    public void onServerSwitch(ServerConnectedEvent event) {
        try {
            initConnection((ServerConnection) serverConnectionField.get(event.getPlayer()));
        } catch (Exception e) {
    ...
    private void initConnection(InboundConnection connection) throws IllegalAccessException {
        ...
        if (connection instanceof InitialInboundConnection) {
            pipeline = ((MinecraftConnection)minecraftConnectionField.get(connection)).getChannel().pipeline();
