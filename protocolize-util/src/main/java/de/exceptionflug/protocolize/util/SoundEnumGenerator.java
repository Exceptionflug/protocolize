            ...
            for(final Sound sound : Sound.values()) {
                ...
                for(final SoundHelper soundHelper : SoundHelper.values()) {
                    if(sound.name().equals(soundHelper.name())) {
                        printWriter.println("\t"+sound.name()+"(new SoundIDMapping(MINECRAFT_1_8, MINECRAFT_1_8, \""+soundHelper.getOldSound()+"\"), new SoundIDMapping(MINECRAFT_1_9, MINECRAFT_1_13_2, \""+soundHelper.getNewSound()+"\")),");
                        found = true;
                        break;
                    }
                }
                if(!found) {
                    printWriter.println("\t"+sound.name()+"(new SoundIDMapping(MINECRAFT_1_9, MINECRAFT_1_13_2, \""+sound.name().toLowerCase().replace("_", ".")+"\")),");
                }
            }
            printWriter.println();
            printWriter.println();
            printWriter.println("\tprivate final SoundIDMapping[] mappings;");
            printWriter.println();
            printWriter.println("\tSound(final SoundIDMapping... mappings) {");
            printWriter.println("\t\tthis.mappings = mappings;");
            printWriter.println("\t}");
            printWriter.println();
            printWriter.println("\tpublic String getSoundName(final int protocolVersion) {");
            printWriter.println("\t\tfor(final SoundIDMapping mapping : mappings) {");
